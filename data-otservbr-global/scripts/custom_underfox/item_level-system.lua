local config = {
    ...
    stonesId = {
        ...
        ["36600"] = 450,
        ["36599"] = 600
    } -- [itemId] = chance -- CHANCE WILL DECREASE PER LEVEL, CALCULATE: CHANCE / LEVEL
}
...
function refiner.onUse(player, item, fromPosition, itemEx, toPosition, isHotkey)
    ...
    elseif slot >= 0 and slot <= 6 and not boolMath then
        ...
        if currentLevel <= 0 then
            ...
            if slot == 0 then
                itemEx:setAttribute(ITEM_ATTRIBUTE_ARMOR, armor + currentLevel - 1)
                return true
            end
            if ((slot >= 1 and slot <= 3) or slot == 5) then
                itemEx:setAttribute(ITEM_ATTRIBUTE_ATTACK, attack + currentLevel - 1)
                return true
            end
            if slot == 4 then
                itemEx:setAttribute(ITEM_ATTRIBUTE_DEFENSE, defense + currentLevel - 1)
                return true
            end
        end
    else
...
refiner:id(170, 30194, 32093, 36600, 36599)
refiner:register()
