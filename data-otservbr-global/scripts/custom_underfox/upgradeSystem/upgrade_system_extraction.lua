...
function upgradeSystem_extraction.onUse(player, item, fromPosition, target, toPosition, isHotkey, tilePosition)
    ...
    local newArmor = (currentUpgrade > 0) and (currentArmor + Upgrade_System_config.addArmor) or (armor + Upgrade_System_config.addArmor)
    local newAttack = (currentUpgrade > 0) and (currentAttack + Upgrade_System_config.addAttack) or (attack + Upgrade_System_config.addAttack)
    local newHitChance = (currentUpgrade > 0) and (currentHitChance + Upgrade_System_config.addHitChance) or (hitChance + Upgrade_System_config.addHitChance)
    local newElement = (currentUpgrade > 0) and (currentElement + Upgrade_System_config.addElement) or (elementDamage + Upgrade_System_config.addElement)
    local newDefense = (currentUpgrade > 0) and (currentDefense + Upgrade_System_config.addDefense) or (defense + Upgrade_System_config.addDefense)
    local newMagicAttack = (currentUpgrade > 0) and (currentMagicAttack + Upgrade_System_config.addAttack) or (attack + Upgrade_System_config.addAttack)
    ...
    if currentUpgrade > 0 and item:getCustomAttribute("tool") == "extract tool" then
        -- Removida a verificação de rank
        ...
        if armor > 0 and Upgrade_System_config.addArmor > 0 or itemType:getType() == ITEM_TYPE_HELMET or itemType:getType() == ITEM_TYPE_ARMOR or itemType:getType() == ITEM_TYPE_LEGS or itemType:getType() == ITEM_TYPE_BOOTS or itemType:getType() == ITEM_TYPE_AMULET or itemType:getType() == ITEM_TYPE_RING then
            armorDescription = ""
            itemAdd:setAttribute(ITEM_ATTRIBUTE_ARMOR, armor)
            itemAdd:setCustomAttribute("armor", 0)
        end
        if attack > 0 and Upgrade_System_config.addAttack > 0 or itemType:getType() == ITEM_TYPE_CLUB or itemType:getType() == ITEM_TYPE_SWORD or itemType:getType() == ITEM_TYPE_AXE or itemType:getType() == ITEM_TYPE_DISTANCE then
            attackDescription = ""
            itemAdd:setAttribute(ITEM_ATTRIBUTE_ATTACK, attack)
            itemAdd:setCustomAttribute("attack", 0)
        end
        if hitChance > 0 and Upgrade_System_config.addHitChance > 0 or itemType:getType() == ITEM_TYPE_DISTANCE then
    ...
    elseif currentUpgrade < 1 and item:getCustomAttribute("tool") == "condensed tool" then
        ...
        itemAdd:setName(newName)
            if armor > 0 and Upgrade_System_config.addArmor > 0 or itemType:getType() == ITEM_TYPE_HELMET or itemType:getType() ==
                ...
                armorDescription = "Arm: +" .. Upgrade_System_config.addArmor * item:getCustomAttribute("upgrade") .. ""
                itemAdd:setAttribute(ITEM_ATTRIBUTE_ARMOR, armor + (Upgrade_System_config.addArmor * item:getCustomAttribute("upgrade")))
            end
            if attack > 0 and Upgrade_System_config.addAttack > 0 or itemType:getType() == ITEM_TYPE_CLUB or itemType:getType() ==
                ...
                attackDescription = "Atk: +" .. Upgrade_System_config.addAttack * item:getCustomAttribute("upgrade") .. ""
                itemAdd:setAttribute(ITEM_ATTRIBUTE_ATTACK, attack + (Upgrade_System_config.addAttack * item:getCustomAttribute("upgrade")))
            end
            if hitChance > 0 and Upgrade_System_config.addHitChance > 0 or itemType:getType() == ITEM_TYPE_DISTANCE and
                item:getCustomAttribute("hitChance") > 0 then
                hitChanceDescription = " Hit%: +" .. Upgrade_System_config.addHitChance * item:getCustomAttribute("upgrade") .. ""
                itemAdd:setAttribute(ITEM_ATTRIBUTE_HITCHANCE, hitChance + (Upgrade_System_config.addHitChance * item:getCustomAttribute("upgrade")))
            end
            if elementDamage > 0 and Upgrade_System_config.addElement > 0 and item:getCustomAttribute("element") > 0 then
                elementDamageDescription = " " .. attackTypeName .. ": +" .. Upgrade_System_config.addElement * item:getCustomAttribute("upgrade") .. ""
                itemAdd:setAttribute(ITEM_ATTRIBUTE_ELEMENT, elementDamage + (Upgrade_System_config.addElement * item:getCustomAttribute("upgrade")))
            end
            if defense > 0 and Upgrade_System_config.addDefense > 0 or itemType:getType() == ITEM_TYPE_SHIELD and
                item:getCustomAttribute("defense") > 0 then
                defenseDescription = " Def: +" .. Upgrade_System_config.addDefense * item:getCustomAttribute("upgrade") .. ""
                itemAdd:setAttribute(ITEM_ATTRIBUTE_DEFENSE, defense + (Upgrade_System_config.addDefense * item:getCustomAttribute("upgrade")))
            end
            if itemType:getType() == ITEM_TYPE_WAND and item:getCustomAttribute("magicAttack") > 0 then
                magicAttackDescription = "Magic Attack: +" .. Upgrade_System_config.addMagicAttack * item:getCustomAttribute("upgrade") .. ""
                itemAdd:setCustomAttribute("magicAttack", currentMagicAttack + (Upgrade_System_config.addMagicAttack * item:getCustomAttribute("upgrade")))
            end
...
upgradeSystem_extraction:aid(Upgrade_System_config.extractiontool)
