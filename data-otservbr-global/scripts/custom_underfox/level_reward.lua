local table = 
{
	[20] = {type = "item", id = {3043, 2}, msg = "You win 2 crystal coins for reach the level 20! Say !promote to get promoted"},
...
function levelReward.onAdvance(player, skill, oldLevel, newLevel)
	...
	for level, _ in pairs(table) do
		if newLevel >= level and player:getStorageValue(storage) < level then
			...
			player:sendTextMessage(TALKTYPE_MONSTER_SAY, table[level].msg)
